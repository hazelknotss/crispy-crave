...
export default async function AdminWastePage({ searchParams }: { searchParams: Search }) {
  ...
  if (scope !== null) {
    if (Number.isFinite(shopId) && shopId !== scope) {
      redirect(`/admin/waste?shop_id=${scope}`);
    }
    shopId = scope;
  }
  ...
  return (
    <>
      <header className="staff-page-head">
        <h1 className="staff-page-head__title">Waste log</h1>
        <p className="staff-page-head__sub">Legacy <code>admin/waste.php</code> stub.</p>
      </header>
