...
export default async function AdminInventoryPage({ searchParams }: { searchParams: Search }) {
  ...
  if (scope !== null) {
    if (Number.isFinite(shopId) && shopId !== scope) {
      redirect(`/admin/inventory?shop_id=${scope}`);
    }
    shopId = scope;
  }
  ...
  return (
    <>
      <header className="staff-page-head">
        <h1 className="staff-page-head__title">Stock / inventory</h1>
        <p className="staff-page-head__sub">
          Legacy <code>admin/inventory.php</code> — ingredients, counts, and alerts will connect to
          Supabase next.
        </p>
      </header>
